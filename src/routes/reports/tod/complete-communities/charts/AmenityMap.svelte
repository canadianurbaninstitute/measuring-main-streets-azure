<script lang="ts">
	...
	// 1. Structure explicit custom interfaces
	interface Region {
		name: string;
	...
	// 2. Assign strong Mapbox and DOM container typings
	...
	onMount(() => {
		...
		map.on('load', () => {
			...
			stationLayers.forEach((layer) => {
				...
				// 3. Cast 'e' directly into a MapLayerMouseEvent to read geo properties safely
				map.on('mousemove', layer, (e: mapboxgl.MapLayerMouseEvent) => {
					if (!map) return;
...
<style>
	/* Styling setup remains perfectly intact */
	.map-wrapper {
		width: 100%;
		height: 100%;
		position: relative;
